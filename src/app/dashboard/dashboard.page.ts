  ...
  async exportToPDF(topic: LearningTopic) {
    ...
    const printContent = `
      ...
      <body>
        <h1>${topic.title}</h1>
        <div class="meta">
          <p><strong>Category:</strong> ${this.getCategoryLabel(
            topic.category
          )}</p>
          <p><strong>Goal:</strong> ${topic.learningGoal || 'Not specified'}</p>
          <p><strong>Progress:</strong> ${this.getCompletedSteps(topic)}/${
          ...
          <div class="step ${step.completed ? 'completed' : ''}">
            ...
            ${
              ...
              <div class="resources">
                ...
                ${step.resources
                  .map((r) => `<div class="resource">• ${r.title}</div>`)
                  .join('')}
              </div>
            `
                : ''
            ...
            ${
              ...
              <div class="notes">
                <strong>Notes:</strong>
                ${step.notes
                  .map((n) => `<div class="note">• ${n.content}</div>`)
                  .join('')}
              </div>
            `
                : ''
